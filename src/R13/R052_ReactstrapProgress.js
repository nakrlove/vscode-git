...
class R052_ReactstrapProgress extends Component {
  ...
  progress =()=>{
    if(this.state.progress !== 100){
      setTimeout(function() {
        this.setState({progress: (this.state.progress < 100 ? this.state.progress+1: this.state.progress) });
        this.progress();
      }.bind(this), 100);
    }
  }
  ...
  render() {
    return (
      <>
        <Progress color="info" bar value={this.state.progress}>{this.state.progress}%</Progress><br/>
        {/* <Progress multi>
